...
export function VideoSettingsPage({ projectId }: { projectId: string }) {
  ...
  const isDirty = Boolean(draft && savedSettings && JSON.stringify(draft) !== JSON.stringify(savedSettings));

  function updateAudio<K extends keyof TTSSettings>(field: K, value: TTSSettings[K]) {
    setDraft((current) => current ? { ...current, audio: { ...current.audio, [field]: value } } : current);
    setNotice("");
  }
  ...
  function updateSubtitleEnabled(enabled: boolean) {
    setDraft((current) => current ? { ...current, subtitle: { ...current.subtitle, enabled } } : current);
    setNotice("");
  }
  ...
  async function handleSave() {
    ...
    setNotice("");
    try {
      const saved = await apiClient.updateVideoSettings(projectId, draft);
  ...
  const style = draft?.subtitle.style;
      ...
      <div className="video-settings-page" aria-labelledby="video-settings-title">
        <header className="video-settings-page__heading">
          ...
          <div className="video-settings-page__scope">
            ...
            <span>전체 {cuts.length}컷에 적용</span>
          </div>
        </header>
        ...
        {!isLoading && draft ? (
          <>
            <div className="video-settings-layout">
              <section className="video-settings-preview-card" aria-labelledby="video-settings-preview-title">
                ...
                <div className={`video-settings-preview__canvas video-settings-preview__canvas--${targetAspectRatio === "1:1" ? "square" : "portrait"}`}>
                  ...
                  {draft.subtitle.enabled && activeCut?.subtitle ? <span className={`video-settings-preview__subtitle video-settings-preview__subtitle--${style?.position ?? "bottom"}`} style={previewStyle}>{activeCut.subtitle}</span> : null}
                </div>
              ...
              <div className="video-settings-form">
                <section className="video-settings-panel" id="video-settings-audio" aria-labelledby="video-settings-audio-title">
                  <div className="video-settings-panel__heading"><div><span>프로젝트 기본값</span><h2 id="video-settings-audio-title">음성 설정</h2></div><span className="video-settings-panel__badge">TTS</span></div>
                  <label className="video-settings-toggle"><input type="checkbox" checked={draft.audio.enabled} onChange={(event) => updateAudio("enabled", event.target.checked)} /><span><strong>TTS 사용</strong><small>내레이션 텍스트를 음성으로 변환합니다.</small></span></label>
                  <div className="video-settings-field-grid">
                    <label className="video-settings-field"><span>음성 엔진</span><select aria-label="음성 엔진" value={draft.audio.provider} onChange={(event) => updateAudio("provider", event.target.value as TTSProvider)}><option value="edge_tts">{TTS_PROVIDER_LABELS.edge_tts}</option><option value="upload">{TTS_PROVIDER_LABELS.upload}</option><option value="azure_speech" disabled>{TTS_PROVIDER_LABELS.azure_speech}</option><option value="elevenlabs" disabled>{TTS_PROVIDER_LABELS.elevenlabs}</option></select></label>
                    <label className="video-settings-field"><span>언어</span><select aria-label="음성 언어" value={draft.audio.language} onChange={(event) => updateAudio("language", event.target.value)}><option value="ko-KR">한국어 · ko-KR</option><option value="en-US">영어 · en-US</option><option value="ja-JP">일본어 · ja-JP</option></select></label>
                    <label className="video-settings-field video-settings-field--wide"><span>목소리</span><select aria-label="목소리" value={draft.audio.voice_id} onChange={(event) => updateAudio("voice_id", event.target.value)}>{VOICE_OPTIONS.map((voice) => <option value={voice.value} key={voice.value}>{voice.label}</option>)}{VOICE_OPTIONS.every((voice) => voice.value !== draft.audio.voice_id) ? <option value={draft.audio.voice_id}>{draft.audio.voice_id}</option> : null}</select></label>
                  </div>
                  <div className="video-settings-range-grid">
                    <label className="video-settings-range"><span>말하기 속도 <output>{draft.audio.speed.toFixed(2)}x</output></span><input aria-label="말하기 속도" type="range" min="0.7" max="1.3" step="0.05" value={draft.audio.speed} onChange={(event) => updateAudio("speed", Number(event.target.value))} /></label>
                    <label className="video-settings-range"><span>음성 볼륨 <output>{Math.round(draft.audio.volume * 100)}%</output></span><input aria-label="음성 볼륨" type="range" min="0" max="1" step="0.05" value={draft.audio.volume} onChange={(event) => updateAudio("volume", Number(event.target.value))} /></label>
                    <label className="video-settings-range"><span>피치 <output>{draft.audio.pitch > 0 ? "+" : ""}{draft.audio.pitch} st</output></span><input aria-label="피치" type="range" min="-12" max="12" step="1" value={draft.audio.pitch} onChange={(event) => updateAudio("pitch", Number(event.target.value))} /></label>
                  </div>
                ...
                <section className="video-settings-panel" id="video-settings-subtitle" aria-labelledby="video-settings-subtitle-title">
                  <div className="video-settings-panel__heading"><div><span>프로젝트 기본값</span><h2 id="video-settings-subtitle-title">자막 설정</h2></div><span className="video-settings-panel__badge">SUBTITLE</span></div>
                  <label className="video-settings-toggle"><input type="checkbox" checked={draft.subtitle.enabled} onChange={(event) => updateSubtitleEnabled(event.target.checked)} /><span><strong>자막 사용</strong><small>미리보기에서 현재 컷의 자막 위치를 확인합니다.</small></span></label>
                  <fieldset className="video-settings-fieldset"><legend>자막 위치</legend><div className="video-settings-position-grid">{POSITION_OPTIONS.map((option) => <button className={`video-settings-position${draft.subtitle.style.position === option.value ? " is-selected" : ""}`} type="button" aria-label={`자막 위치: ${option.label}`} aria-pressed={draft.subtitle.style.position === option.value} key={option.value} onClick={() => updateSubtitle("position", option.value)}>{option.label}</button>)}</div></fieldset>
                  {draft.subtitle.style.position === "custom" ? <div className="video-settings-field-grid"><label className="video-settings-field"><span>가로 위치 (%)</span><input aria-label="자막 가로 위치" type="number" min="0" max="100" value={draft.subtitle.style.custom_x} onChange={(event) => updateSubtitle("custom_x", Number(event.target.value))} /></label><label className="video-settings-field"><span>세로 위치 (%)</span><input aria-label="자막 세로 위치" type="number" min="0" max="100" value={draft.subtitle.style.custom_y} onChange={(event) => updateSubtitle("custom_y", Number(event.target.value))} /></label></div> : null}
                  <div className="video-settings-field-grid">
                    <label className="video-settings-field"><span>글꼴</span><select aria-label="자막 글꼴" value={draft.subtitle.style.font_family} onChange={(event) => updateSubtitle("font_family", event.target.value)}>{FONT_OPTIONS.map((font) => <option value={font} key={font}>{font}</option>)}{FONT_OPTIONS.every((font) => font !== draft.subtitle.style.font_family) ? <option value={draft.subtitle.style.font_family}>{draft.subtitle.style.font_family}</option> : null}</select></label>
                    <label className="video-settings-range"><span>글자 크기 <output>{draft.subtitle.style.font_size}px</output></span><input aria-label="자막 글자 크기" type="range" min="24" max="120" step="1" value={draft.subtitle.style.font_size} onChange={(event) => updateSubtitle("font_size", Number(event.target.value))} /></label>
                    <label className="video-settings-field"><span>정렬</span><select aria-label="자막 정렬" value={draft.subtitle.style.alignment} onChange={(event) => updateSubtitle("alignment", event.target.value as SubtitleStyle["alignment"])}><option value="left">왼쪽</option><option value="center">가운데</option><option value="right">오른쪽</option></select></label>
                    <label className="video-settings-field"><span>최대 줄 수</span><select aria-label="자막 최대 줄 수" value={draft.subtitle.style.max_lines} onChange={(event) => updateSubtitle("max_lines", Number(event.target.value))}>{[1, 2, 3, 4].map((lines) => <option value={lines} key={lines}>{lines}줄</option>)}</select></label>
                  </div>
                  <div className="video-settings-color-grid">
                    <label className="video-settings-color"><span>글자 색상</span><input aria-label="자막 글자 색상" type="color" value={draft.subtitle.style.color} onChange={(event) => updateSubtitle("color", event.target.value)} /></label>
                    <label className="video-settings-color"><span>외곽선 색상</span><input aria-label="자막 외곽선 색상" type="color" value={draft.subtitle.style.outline_color} onChange={(event) => updateSubtitle("outline_color", event.target.value)} /></label>
                    <label className="video-settings-range"><span>외곽선 두께 <output>{draft.subtitle.style.outline_width.toFixed(1)}px</output></span><input aria-label="자막 외곽선 두께" type="range" min="0" max="8" step="0.5" value={draft.subtitle.style.outline_width} onChange={(event) => updateSubtitle("outline_width", Number(event.target.value))} /></label>
                  </div>
                  <div className="video-settings-inline-options">
                    <label className="video-settings-check"><input type="checkbox" checked={draft.subtitle.style.background_color !== null} onChange={(event) => updateSubtitle("background_color", event.target.checked ? "#111111" : null)} /><span>자막 배경</span></label>
                    <input aria-label="자막 배경 색상" type="color" value={draft.subtitle.style.background_color ?? "#111111"} disabled={draft.subtitle.style.background_color === null} onChange={(event) => updateSubtitle("background_color", event.target.value)} />
                    <label className="video-settings-check"><input type="checkbox" checked={draft.subtitle.style.safe_area} onChange={(event) => updateSubtitle("safe_area", event.target.checked)} /><span>안전 영역 표시</span></label>
                  </div>
                </section>
              </div>
            </div>
            ...
            <footer className="video-settings-actions">
              ...
              <button className="button button--primary" type="button" onClick={() => void handleSave()} disabled={!isDirty || isSaving} aria-busy={isSaving}><FloppyDisk size={16} aria-hidden="true" />{isSaving ? "저장 중…" : "설정 저장"}</button>
            </footer>
          </>
        ) : null}
      </div>
